...
public class CalenderAcitity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        datePicker.setOnDateChangedListener(new DatePicker.OnDateChangedListener() {
            @Override
            public void onDateChanged(DatePicker datePicker, int year, int monthYear, int dayMonth) {

                textView.setText(year + "년" + (monthYear+1) + "월" + dayMonth + "일");
            }
        });

        // 하단바
        BottomNavigationView bottomNav = findViewById(R.id.bottom_navigation_view);
        bottomNav.setOnNavigationItemSelectedListener(new CalenderAcitity.ItemSelectListener());
    }
    ...
    private class ItemSelectListener implements BottomNavigationView.OnNavigationItemSelectedListener{
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
            ...
            switch (menuItem.getItemId()) {
                //하단바 - 홈
                case R.id.nav_home:
                    transaction.replace(R.id.CalFrameLayout, homeFragment).commitAllowingStateLoss();
                    break;

                //하단바 - Sitter
                case R.id.nav_shop:
                    transaction.replace(R.id.CalFrameLayout, shopFragment).commitAllowingStateLoss();
                    break;

                //하단바 - Trainer
                case R.id.nav_cart:
                    transaction.replace(R.id.CalFrameLayout, cartFragment).commitAllowingStateLoss();
                    break;
